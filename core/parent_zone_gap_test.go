...
func TestItemsExcludesDelegatedChildLeavesButParentStubRemains(t *testing.T) {
	...
	c.Own(root, domain.Delegation{child: nil})
	c.Own(child, domain.Delegation{})
	idRoot, _ := encoding.MergeEncodings(encoding.BASE64, encoding.BASE64, root, coll)
	idChild, _ := encoding.MergeEncodings(encoding.BASE64, encoding.BASE64, child, coll)

...
# scripts/deploy/scripts → core is three levels up
...
chmod +x "$ROOT/scripts/deploy/scripts/bake-ami.sh"
"$ROOT/scripts/deploy/scripts/bake-ami.sh"
...
# Fast spawn path: baked AMI (binary+systemd). S3 bin/ is bake+bootstrap fallback only — no ECR/Docker.
